...
public class updateOrgUnits_multiThreading {
	
	public updateOrgUnits_multiThreading() {}

	private String DBurl, DBusr, DBpwd, HBaseURL, scannerTRbatch, bodyScanner, useMt, tablesNamesCSV, tablesMultiThread;
	long thisExtractionTS;
	String thisExtractionGV, prevExtractionGV; //are long stored as string
	...
	public void setExtractionTimes(String ctrlPath) throws IOException {
		...
		Properties props = new Properties();
		props.load(in);
		String update = props.getProperty("tz_thisExtractionOrgUnits");
		long longUpdate = Long.parseLong(update);
		Date d = new Date(longUpdate);
		System.out.println("last extraction from Organisation Units from Landing Zone was made on: "+ d);
		in.close();

		FileOutputStream out = new FileOutputStream(ctrlPath);
		//set control.properties variables with global var values 
		props.setProperty("tz_prevExtractionOrgUnits", this.prevExtractionGV);
		props.setProperty("tz_thisExtractionOrgUnits", this.thisExtractionGV);
		long longUpdateNEW = Long.parseLong(this.thisExtractionGV);
		Date dNEW = new Date(longUpdateNEW);
		System.out.println("NEW extraction from Organisation Units Landing Zone table done at: "+ dNEW);
	...
	public void setProperties(String ctrlPath) throws IOException{
		...
		this.setTablesMultiThread(props.getProperty("tablesMultiThread"));
		String[] tablesMT = tablesMultiThread.split(",");
		List tablesList = Arrays.asList(tablesMT);
		if(tablesList.contains("AdministrationUnit")) {
			this.setUseMultiThreading("true");
		}
		else this.setUseMultiThreading("false");
	...
	public String getTablesMultiThread() {return this.tablesMultiThread;}
	public void setTablesMultiThread(String TablesMultiThread) {this.tablesMultiThread = TablesMultiThread;}
